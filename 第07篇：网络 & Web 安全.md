...
2. 第二次握手（**SYN + ACK**）：服务器收到SYN包并确认客户的SYN（*ACK=x+1*），同时发送一个自己的SYN包（*SYN=1,Seq=y*），即SYN + ACK，此时服务器进入 `SYN_RCVD` 状态。

3. 第三次握手（**ACK**）：客户端收到服务器的SYN+ACK包，向服务器发送确认包 ACK（*ACK=y+1*），此时客户端和服务器进入 `ESTABLISHED`（TCP连接成功）状态，完成三次握手。
...
TCP和UDP是两种常见的传输协议，位于OSI模型中的 **传输层**。
...
| 安全性     | 客户端可见，明文存储（不安全）                   | 仅存ID（安全）                                        |
| 服务器压力 | 无                                               | 保存在服务器端，每个用户产生一个Session，占用大量内存 |
| 跨域支持   | ✅                                                | ❌                                                     |

总结：

1. **Cookie**：客户端存储，小容量，跨域，不安全。
2. **Session**：服务器存储，大容量，安全，不跨域。

### 008：HTTP/浏览器缓存机制 ✔️

HTTP 缓存是客户端和服务器之间减少重复请求、提高性能的规则和策略。
...
2. **协商缓存**
   - 未命中强缓存时，携带缓存标识请求服务器：
     ...
     - `If-None-Match`（对比 `ETag`）— 基于资源的唯一标识
   - 资源未变 → `304 Not Modified`，告知浏览器从缓存中获取资源
   - 资源变化 → 返回新资源

一句话总结：

- 强缓存：过期前不请求
- 协商缓存：问服务器"能用旧的吗？"（304=能用，200=用新的）
...
### 015：如何防止网页被他人通过 \<iframe> 嵌套？✔️

1. 使用 `X-Frame-Options` HTTP 头 —— 2009 年，IE8时代，基础防护，逐步淘汰。
2. 使用 `Content-Security-Policy (CSP)` HTTP 头 —— 2012 年（现代浏览器标准），综合防护，W3C推荐标准。
3. js 辅助方案，`window !== window.top` 强制跳出 iframe
4. 启用 https，防止中间人攻击
5. 使用 `SameSite` Cookie，防止 CSRF 攻击。
