...
`$nextTick` 确保在数据变化时，获取到更新之后的DOM。

因为 Vue 的响应式系统是异步的，当数据发生变化时，Vue 并不会立即更新 DOM，而是将更新操作推入一个队列，并在下一个事件循环中批量处理。这就导致在数据变化之后立即访问DOM，可能拿不到更新之后的DOM。

### 007：**scoped** 原理 ✔️

作用：样式隔离，避免全局污染。

原理：给当前组件的所有选择器加上一个唯一的标识符（如 `data-v-xxx`）
...
### 010：computed、methods、watch 有什么区别？
